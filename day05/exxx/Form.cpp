...
Form::Form() 
: _name ("-"), _exec_grade (0), _sign_grade (0), _status (false)
{
}
...
void Form::pre_execute(Bureaucrat & one)
{
    if (one.getNote() > this->_exec_grade)
        throw ExecException();
...
{
    return ("The Bureaucrat's note is too low!"); 
}
